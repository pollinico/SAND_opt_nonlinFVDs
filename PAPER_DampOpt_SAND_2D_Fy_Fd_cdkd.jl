...
resultsFolder = "./res_2dof_Fy_Fd_cdkd/"
folder = "./2dof_ex/"
...
β  = 1/4.0 # Newmark's parameters
γ  = 1/2.0
Δt = t[2] - t[1]
dmax  = 15.0 # mm, max drift
dmaxVec = [13.0, 11.0, 9.0]
cdMax = 100.0 # 100.0 # Max damping coefficient in kNs/mm
h = 3000.0 # mm
l = 5000.0 # mm
...
maxU = 3*dmax/cStateVarU # mm/s
maxUd = dmax/cStateVarU # mm/s
maxV = Inf # 600.0/cStateVarV # mm/s
maxVd = Inf # 500.0/cStateVarV # mm/s
maxA = Inf # 15.0e3/cStateVarA # mm/s^2  
maxFd = Inf # 200.0/cFd # kN
maxFs = Inf # 3*maximum(Fy)/cFs # kN # kN
maxdFs = Inf # 5.0e3/cFs #Inf
maxdFd = Inf # 1.0e4/cFd
...
set_optimizer_attribute(model, "output_file", resultsFolder*"ipopt_out_Fy_Fd_cdkd_dMax_"*string(Int(dmax))*".txt")
objHist = Float64[]
gHist   = Float64[]
iterIPOPT = Float64[]
timeIPOPT = Float64[]
...
# Variables:
@variable(model, 1.0e-3 <= xd[1:Ndamper] <= 1.0)
...
@variable(model, -maxdFd <= dFd[1:Ndamper,1:Nstep-1] <= maxdFd)
if !WARM_START
...
@variable(model, 0.0 <= τ <= Inf, start=1e5)
...
@constraint(model, a0, xa[:,1] .== m \ (P[:,1]/cStateVarA - cs*zeros(ndof_u) - k*zeros(ndof_u)))
...
# Time derivative of damper force
@constraint(model, [i=2:Nstep], Fd[:,i] .== Fd[:,i-1] + dFd[:,i-1] * Δt )

# Nonlinear FVDs
for j in 1:Ndamper
    @NLconstraint(model, [i=1:Nstep-1],  dFd[j,i] ==  ρ * cdMax * xd[j] * (cStateVarV*vd[j,i]*sind(θ) - sign(Fd[j,i]) * (abs(cFd*Fd[j,i])/(cdMax*xd[j]))^(1/α)) / cFd )
end

# Equilibrium equations
@constraint(model, eq_eq[i=2:Nstep], (m * xa[:,i] .* cStateVarA + cs * xv[:,i] .* cStateVarV + H' * Fd[:,i] .* cFd .* sind(θ) + T' * Fs[:,i] .* cFs) ./ cEqEq .== P[:,i] ./ cEqEq)
...
Obj_solutions = hcat(Obj_solutions, cFd*objective_value(model))

# Extract data from output file of IPOPT
iter, time = readIpoptOutput(resultsFolder, "ipopt_out_Fy_Fd_cdkd_dMax_"*string(Int(dmax))*".txt")
push!(iterIPOPT, iter)
push!(timeIPOPT, time)
...
fd = cFd*value.(model[:Fd])
fs = cFs*value.(model[:Fs])
dfd = cFd*value.(model[:dFd])
dfs = cFs*value.(model[:dFs])
#alpha = value.(model[:xalpha])
...
println("Final objective function: ", round(f, digits=3), " kN")
println("Final cd1: ", round(Cd[1], digits=3), " kNs/mm")
println("Final cd2: ", round(Cd[2], digits=3), " kNs/mm")
#println("Final α: ", round(alpha, digits=3), " ")
...
println("Final max dfd: ", round(maximum(abs.(dfd)), digits=3), " kN/s")
...
io = open(resultsFolder*"opt_sol_ipopt.txt", "w")
write(io,string("Optimized solution for linear structure with nonlinear dampers with Maxwell model","\n"))
write(io,string("Number of time steps: ", length(t),"\n"))
write(io,string("Time step: ", Δt, " s","\n"))
write(io,string("Final objective function: ", round(f, digits=3), " kN","\n"))
...
write(io,string("Final max dfd: ", round(maximum(abs.(dfd)), digits=3), " kN/s","\n"))
...
plot!(t, drift[2,:], label = "d2", α=0.8, w=2)
...
xtickfont=15, ytickfont=15,  
guidefontsize=15, legendfont=15, margin=4Plots.mm)
plot!(t, u[2,:], label = "u2", α=0.8, w=2)
xlabel!("t [s]")
ylabel!("u [mm]")
savefig(resultsFolder*"opt_u_2D.png")

plot(t,  v[1,:], label = "v1", α=0.8, w=2, size = (800, 400),  
xtickfont=15, ytickfont=15,  
guidefontsize=15, legendfont=15, margin=4Plots.mm)
plot!(t, v[2,:], label = "v2", α=0.8, w=2)
xlabel!("t [s]")
ylabel!("v [mm/s]")
savefig(resultsFolder*"opt_v_2D.png")

plot(t,  a[1,:], label = "a1", α=0.8, w=2, size = (800, 400),  
xtickfont=15, ytickfont=15,  
guidefontsize=15, legendfont=15, margin=4Plots.mm)
plot!(t, a[2,:], label = "a2", α=0.8, w=2)
xlabel!("t [s]")
ylabel!("a [mm/s\$^2\$]")
...
xlabel!("v [mm/s]")
ylabel!("fd1 [kN]")
savefig(resultsFolder*"opt_fd1_2D.png")
...
xlabel!("v [mm/s]")
ylabel!("fd2 [kN]")
savefig(resultsFolder*"opt_fd2_2D.png")
...
plot((cdMax*cStateVarV) * objHist, label = "", α=0.9, w=2, size = (800, 400),  
xtickfont=15, ytickfont=15,  
guidefontsize=15, legendfont=15, margin=4Plots.mm)
xlabel!("iter")
ylabel!("f")
